...
must_haves:
  truths:
    ...
    - "PROVIDER_MODELS table maps abstract tiers to provider-specific model IDs for claude, opencode-claude, opencode-codex, copilot-claude, copilot-codex, and gemini"
...
//
// Switch provider with GSD_RUNTIME env var (e.g. GSD_RUNTIME=copilot-claude)
...
```
  ...
  <done>
MODEL_PROFILES contains only reasoning/standard/fast values. PROVIDER_MODELS has 6 entries: claude→opus/sonnet/haiku, opencode-claude→anthropic/claude-opus-4-6 / anthropic/claude-sonnet-4-5 / anthropic/claude-haiku-4-5, opencode-codex→openai/gpt-5.3-codex for all tiers, copilot-claude→github-copilot/claude-opus-4.6 / github-copilot/claude-sonnet-4.5 / github-copilot/claude-haiku-4.5, copilot-codex→github-copilot/gpt-5.2-codex for all tiers, gemini→gemini-2.5-pro/gemini-2.5-flash/gemini-2.5-flash-lite. detectRuntime() checks GSD_RUNTIME → OPENCODE=1 (returns opencode-claude) → GEMINI_API_KEY/GOOGLE_GENAI_USE_VERTEXAI → CLAUDE_CODE_SSE_PORT → claude fallback.
  </done>
</task>
...
GSD_RUNTIME=copilot-claude node get-shit-done/bin/gsd-tools.js resolve-model gsd-planner --raw
# Expected: github-copilot/claude-opus-4.6

# GitHub Copilot Codex variant
GSD_RUNTIME=copilot-codex node get-shit-done/bin/gsd-tools.js resolve-model gsd-executor --raw
...
```
  ...
  <done>
All 5 success criteria pass: (1) MODEL_PROFILES uses abstract tiers, (2) PROVIDER_MODELS maps tiers to 6 providers (claude, opencode-claude, opencode-codex, copilot-claude, copilot-codex, gemini) using correct provider/model format, (3) runtime auto-detected from env vars (OpenCode defaults to opencode-claude), (4) GSD_MODEL=gpt-5.3-codex returns gpt-5.3-codex, (5) Claude Code default returns opus for quality gsd-planner. Copilot variants use github-copilot/ prefix with dot-separated versions. Unknown agent fallback returns provider-equivalent of standard tier. --raw output contract preserved (single model string).
  </done>
</task>
...
GSD_RUNTIME=copilot-claude node get-shit-done/bin/gsd-tools.js resolve-model gsd-planner --raw
# Expected: github-copilot/claude-opus-4.6 (copilot-claude reasoning tier)

GSD_RUNTIME=copilot-codex node get-shit-done/bin/gsd-tools.js resolve-model gsd-planner --raw
# Expected: github-copilot/gpt-5.2-codex (copilot-codex maps all tiers to codex)
...
2. `grep 'PROVIDER_MODELS' gsd-tools.js` confirms table exists with claude, opencode-claude, opencode-codex, copilot-claude, copilot-codex, gemini providers
