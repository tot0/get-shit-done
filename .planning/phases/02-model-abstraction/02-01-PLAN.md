...
must_haves:
  truths:
    ...
    - "PROVIDER_MODELS table maps abstract tiers to provider-specific model IDs for claude, opencode-claude, opencode-codex, and gemini"
...
Purpose: Enable gsd-tools.js to return provider-appropriate model IDs (opus for Claude Code, anthropic/claude-opus-4-6 for OpenCode+Claude, openai/gpt-5.3-codex for OpenCode+Codex, gemini-2.5-pro for Gemini) from a single abstract tier system (reasoning/standard/fast), with GSD_MODEL env var override for per-invocation model switching. OpenCode has two provider sub-variants: `opencode-claude` (default, uses provider/model format) and `opencode-codex` (all tiers → openai/gpt-5.3-codex).
...
2. **Add PROVIDER_MODELS table** immediately after MODEL_PROFILES (before the `// ─── Helpers` section). Include a section comment. OpenCode has TWO sub-variants: `opencode-claude` (default, using Claude models via OpenCode) and `opencode-codex` (all tiers map to gpt-5.3-codex). Runtime detection returns `opencode-claude` by default; users switch to `opencode-codex` via `GSD_RUNTIME=opencode-codex`:

```javascript
// ─── Provider Model Mapping ──────────────────────────────────────────────────
// OpenCode uses provider/model format (e.g. anthropic/claude-opus-4-6)
// Claude Code uses shorthand (e.g. opus)
// Gemini CLI uses model names (e.g. gemini-2.5-pro)
...
# PROVIDER_MODELS table exists with all 4 provider entries
grep -A 30 'const PROVIDER_MODELS' get-shit-done/bin/gsd-tools.js
...
```
  ...
  <done>
MODEL_PROFILES contains only reasoning/standard/fast values. PROVIDER_MODELS maps claude→opus/sonnet/haiku, opencode-claude→anthropic/claude-opus-4-6 / anthropic/claude-sonnet-4-5 / anthropic/claude-haiku-4-5, opencode-codex→openai/gpt-5.3-codex for all tiers, gemini→gemini-2.5-pro/gemini-2.5-flash/gemini-2.5-flash-lite. detectRuntime() checks GSD_RUNTIME → OPENCODE=1 (returns opencode-claude) → GEMINI_API_KEY/GOOGLE_GENAI_USE_VERTEXAI → CLAUDE_CODE_SSE_PORT → claude fallback.
  </done>
</task>
...
```
  ...
  <done>
All 5 success criteria pass: (1) MODEL_PROFILES uses abstract tiers, (2) PROVIDER_MODELS maps tiers to 4 providers (claude, opencode-claude, opencode-codex, gemini) using correct provider/model format for OpenCode, (3) runtime auto-detected from env vars (OpenCode defaults to opencode-claude), (4) GSD_MODEL=gpt-5.3-codex returns gpt-5.3-codex, (5) Claude Code default returns opus for quality gsd-planner. OpenCode-codex returns openai/gpt-5.3-codex for all tiers. Unknown agent fallback returns provider-equivalent of standard tier. --raw output contract preserved (single model string).
  </done>
</task>
...
# SC2: PROVIDER_MODELS table exists with all 4 provider entries
grep -c 'PROVIDER_MODELS' get-shit-done/bin/gsd-tools.js
# Expected: >= 4 (declaration + usages)
...
2. `grep 'PROVIDER_MODELS' gsd-tools.js` confirms table exists with claude, opencode-claude, opencode-codex, gemini providers
