...
Purpose: Enable gsd-tools.js to return provider-appropriate model IDs (opus for Claude Code, opus-4.6/sonnet-4.5/haiku-4.5 for OpenCode+Claude, gpt-5.3-codex for OpenCode+Codex, gemini-2.5-pro for Gemini) from a single abstract tier system (reasoning/standard/fast), with GSD_MODEL env var override for per-invocation model switching. OpenCode has two provider sub-variants: `opencode-claude` (default) for Claude models and `opencode-codex` for Codex models (all tiers → gpt-5.3-codex).
...
```
  ...
  <done>
MODEL_PROFILES contains only reasoning/standard/fast values. PROVIDER_MODELS maps claude→opus/sonnet/haiku, opencode-claude→opus-4.6/sonnet-4.5/haiku-4.5, opencode-codex→gpt-5.3-codex for all tiers, gemini→gemini-2.5-pro/gemini-2.5-flash/gemini-2.0-flash-lite. detectRuntime() checks GSD_RUNTIME → OPENCODE=1 (returns opencode-claude) → GEMINI_API_KEY/GOOGLE_GENAI_USE_VERTEXAI → CLAUDE_CODE_SSE_PORT → claude fallback.
  </done>
</task>
...
# NOTE: If running under OpenCode, expected output is opus-4.6 instead (opencode-claude)
...
# Expected: sonnet (on Claude Code) or sonnet-4.5 (on OpenCode)
...
# OpenCode Codex variant — all tiers map to gpt-5.3-codex
...
```
  ...
  <done>
All 5 success criteria pass: (1) MODEL_PROFILES uses abstract tiers, (2) PROVIDER_MODELS maps tiers to 4 providers (claude, opencode-claude, opencode-codex, gemini), (3) runtime auto-detected from env vars (OpenCode defaults to opencode-claude), (4) GSD_MODEL=gpt-5.3-codex returns gpt-5.3-codex, (5) Claude Code default returns opus for quality gsd-planner. OpenCode-codex returns gpt-5.3-codex for all tiers. Unknown agent fallback returns provider-equivalent of standard tier. --raw output contract preserved (single model string).
  </done>
</task>
...
node get-shit-done/bin/gsd-tools.js resolve-model gsd-planner --raw
# Expected: opus (on Claude Code) or opus-4.6 (on OpenCode)
...
# Expected: sonnet-4.5 (opencode-claude standard tier)

GSD_RUNTIME=opencode-codex node get-shit-done/bin/gsd-tools.js resolve-model gsd-executor --raw
# Expected: gpt-5.3-codex (opencode-codex maps all tiers to codex)
